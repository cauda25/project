...
public interface MovieRepository extends JpaRepository<Movie, Long>, MovieCustomRepository {
    ...
    // @Query("SELECT m FROM Movie m JOIN m.movieGenres mg JOIN mg.genre g WHERE g
    // IN :genres")
    // List<Movie> findMoviesByGenres(List<Genre> genres);

    // 해당 감독의 영화 리스트
    // @Query("SELECT m FROM Movie m JOIN m.moviePeople mp JOIN mp.person p WHERE
    // p.id = :directorId AND p.job = 'Directing'")
